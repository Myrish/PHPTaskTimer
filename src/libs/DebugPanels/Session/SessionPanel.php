...
class SessionPanel extends \Nette\Application\UI\Control implements \Nette\Diagnostics\IBarPanel
{
	 ...
	 * @return void
	 */
	public function __construct(\Nette\Application\Application $application, \Nette\Http\Session $session, \Nette\Http\Request $request)
	{
        ...
        $sessionKeyToRemove = $request->getQuery("session-panel-remove-session", null);
        if($sessionKeyToRemove)
        {
            $this->removeSession($sessionKeyToRemove);

            $this->redirectUrl($request);
        }
	}

    public function redirectUrl(\Nette\Http\Request $request)
    {
        $response = new \Nette\Http\Response;

        $query = $request->getQuery();
        unset($query['session-panel-remove-session']);
	...
	public function templatePrepareFilters($template)
	{
        ...
        $set->addMacro('remove-session-href', NULL, NULL, function($node, $writer) use($request) {
            $query = $request->getQuery();
            $query['session-panel-remove-session'] = "";
            $url = clone $request->getUrl();
            $url->setQuery($query);
    ...
    public function removeSession($sectionKey)
    {
        $this->session->getSection($sectionKey)->remove();
    }
}
